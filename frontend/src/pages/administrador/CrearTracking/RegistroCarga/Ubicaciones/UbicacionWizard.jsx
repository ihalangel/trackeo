...
export default function UbicacionWizard({ index }) {
  const {
    ...
    lugares,
    accionesUbicacion,
    setMostrarResumenGlobal, // Nueva función que debe venir del contexto o prop para controlar mostrar resumen global
  } = useTracking();
  ...
  // Detectar si todas las ubicaciones están finalizadas para mostrar resumen global
  useEffect(() => {
    if (form.ubicaciones.every((u) => u.finalizado)) {
      if (setMostrarResumenGlobal) setMostrarResumenGlobal(true);
    }
  }, [form.ubicaciones, setMostrarResumenGlobal]);
